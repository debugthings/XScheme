...
- (CGFloat)height {
    CGFloat height = kHeightForCell + [self numberOfRows] * kHeightForCell + [self numberOfRows] * kIndentBetweenCells;
    
    if ([self isAllowNewConnection])
        height += kHeightForNewConnectionButton + kIndentBetweenCells;
...
- (XSNewConnectionButton *)newConnectionButton {
    if (!_newConnectionButton) {
        ...
        _newConnectionButton.title = @"Новое подключение";
    }
